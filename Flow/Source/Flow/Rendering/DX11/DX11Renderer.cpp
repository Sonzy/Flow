...
Flow::DX11Renderer::DX11Renderer(HWND WindowHandle, int ViewportWidth, int ViewportHeight)
{
...
void Flow::DX11Renderer::ClearWindow(float R, float G, float B, float A)
{
...
void Flow::DX11Renderer::EndFrame()
{
	HRESULT ResultHandle;
	CATCH_ERROR_DX(SwapChain->Present(1, 0));
}

ID3D11DeviceContext* Flow::DX11Renderer::GetContext()
{
	return Context.Get();
}

ID3D11Device* Flow::DX11Renderer::GetDevice()
{
	return Device.Get();
}
